...
odoo.define('dibon_mpesa.payment_form', function (require) {
    'use strict';
    ...
    PaymentForm.include({
        events: _.extend({}, PaymentForm.prototype.events, {
            'submit': '_onSubmit',
            'click .o_payment_submit_button': '_onSubmitClicked'
        }),

        _onSubmitClicked: function (ev) {
            if (this._ismpesaPayment()) {
                ev.preventDefault();
                return this._processMpesaPayment();
            }
            return this._super.apply(this, arguments);
        },

        _ismpesaPayment: function() {
            var radio = this.$('input[name="o_payment_radio"]:checked');
            return radio.length && radio.data('provider') === 'mpesa_online';
        },

        _processMpesaPayment: function () {
            var self = this;
            var form = this.$el;
            
            // Validate form
            if (!this._validateMpesaForm()) {
                return false;
            }
            ...
            var formData = {
                ...
                'currency_id': form.find('input[name="currency_id"]').val(),
                'partner_id': form.find('input[name="partner_id"]').val(),
                'access_token': form.find('input[name="access_token"]').val(),
                'provider_id': form.find('input[name="provider_id"]').val(),
                'mpesa_receipt': form.find('input[name="mpesa_receipt"]').val(),
                'mpesa_phone_number': form.find('input[name="mpesa_phone_number"]').val(),
                'payment_method': form.find('input[name="payment_method"]').val(),
                'manual_payment': form.find('input[name="payment_method"]').val() === 'manual_entry'
            };

            // Disable submit button
            this.disableButton(this.$('.o_payment_submit_button'));

            // Submit payment
            return this._rpc({
                route: '/payment/mpesa_online/submit',
                params: formData
            }).then(function (result) {
                if (result.success) {
                    window.location = '/payment/status';
                } else {
                    self.enableButton(self.$('.o_payment_submit_button'));
                    self.displayError(
                        _t('Payment Error'),
                        result.error || _t('Could not process payment.')
                    );
                }
                ...
                self.displayError(
                    _t('Server Error'),
                    _t('Could not process payment. Please try again.')
                );
            });
        },

        _validateMpesaForm: function() {
            var phoneNumber = this.$('input[name="mpesa_phone_number"]').val();
            var receipt = this.$('input[name="mpesa_receipt"]').val();
            var isManual = this.$('input[name="payment_method"]').val() === 'manual_entry';

            if (!phoneNumber) {
                this.displayError(_t('Validation Error'), _t('Please enter phone number'));
                return false;
            }

            if (isManual && !receipt) {
                this.displayError(_t('Validation Error'), _t('Please enter MPesa receipt number'));
                return false;
            }
