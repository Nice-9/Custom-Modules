...
class ProductProduct(models.Model):
    ...
    qr_sequence = fields.Char(string="QR Sequence", readonly=True,
                              help='to add QR sequence')
    qr = fields.Binary(string="QR Code", help='Qr file')

    @api.model
    def create(self, vals):
        """Create a new product and assign a unique QR sequence and QR code to it."""
    ...
    def generate_qr(self):
        """Generate a QR code based on the product's qr_sequence and store it in
        ...
        if qrcode and base64:
            ...
            return self.env.ref(
                'customer_product_qrcode.print_qr2').report_action(self, data={'data': self.id, 'type': 'prod'})
        else:
            raise UserError(
    ...
    def get_product_by_qr(self, **args):
        ...
        return self.env['product.product'].search(
            [('qr_sequence', '=', self.id), ], limit=1).id
