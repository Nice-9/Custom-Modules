...
class SurveyUserInputLine(models.Model):
    ...
    answer_type = fields.Selection(
        selection_add=[('url', 'URL'),
                       ...
                       ('qr', 'Qrcode'),
                       ('barcode', 'Barcode'),
                       ('file', 'Upload file')], help="Custom answer types")
    ...
    value_quantity = fields.Float(related="suggested_answer_id.quantity", store=True)
    value_product_id = fields.Many2one(related="suggested_answer_id.product_id", store=True)
    value_upload_file = fields.Many2one()
        ...
        for line in self:
            if line.answer_type == 'char_box':
                ...
                if line.value_file_data_ids:
                    attachment_names = ','.join(line.value_file_data_ids.mapped("name"))
                else:
                    attachment_names = ""
                line.display_name = _('Attachments: ' + attachment_names)
